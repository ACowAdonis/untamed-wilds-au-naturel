...
public class FishReturnToSchoolGoal extends RandomSwimmingGoal {
   ...
   public boolean canUse() {
      return this.mob instanceof IPackEntity
            && this.taskOwner.herd != null
            && this.taskOwner.herd.getLeader() != this.taskOwner
            && !(this.taskOwner.distanceTo(this.taskOwner.herd.getLeader()) < (float)this.maxDist)
         ? super.canUse()
         : false;
   }
   ...
   public void tick() {
      super.tick();
      if (this.taskOwner.distanceTo(this.taskOwner.herd.getLeader()) < (float)this.maxDist) {
         ...
         if (dest != null) {
            this.taskOwner.getNavigation().moveTo((double)dest.getX(), (double)dest.getY(), (double)dest.getZ(), this.speedModifier / 2.0);
         }
      }
   }
