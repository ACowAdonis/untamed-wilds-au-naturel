...
public class JumpGoal extends Goal {
   ...
   private boolean canJumpTo(BlockPos pos, int dx, int dz, int scale) {
      ...
      return this.taskOwner.level().getFluidState(blockpos).is(FluidTags.WATER) && !this.taskOwner.level().getBlockState(blockpos).blocksMotion();
   }

   private boolean isAirAbove(BlockPos pos, int dx, int dz, int scale) {
      return this.taskOwner.level().getBlockState(pos.offset(dx * scale, 1, dz * scale)).isAir()
         && this.taskOwner.level().getBlockState(pos.offset(dx * scale, 2, dz * scale)).isAir();
   }

   public boolean canContinueToUse() {
      double d0 = this.taskOwner.getDeltaMovement().y;
      return (!(d0 * d0 < 0.03F) || this.taskOwner.getXRot() == 0.0F || !(Math.abs(this.taskOwner.getXRot()) < 10.0F) || !this.taskOwner.isInWater())
   ...
   public void start() {
      Direction direction = this.taskOwner.getMotionDirection();
      this.taskOwner.setDeltaMovement(this.taskOwner.getDeltaMovement().add((double)direction.getStepX() * 0.6, 0.6, (double)direction.getStepZ() * 0.6));
   ...
   public void tick() {
      ...
      if (vec3.y * vec3.y < 0.03F && this.taskOwner.getXRot() != 0.0F) {
