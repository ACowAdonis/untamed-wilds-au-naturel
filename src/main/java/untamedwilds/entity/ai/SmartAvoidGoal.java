...
public class SmartAvoidGoal<T extends LivingEntity> extends AvoidEntityGoal<T> {
   ...
   public SmartAvoidGoal(
      ComplexMob entityIn, Class<T> classToAvoidIn, float avoidDistanceIn, double farSpeedIn, double nearSpeedIn, Predicate<LivingEntity> targetSelector
   ) {
   ...
   public boolean canUse() {
      if ((double)(this.taskOwner.getHealth() / this.taskOwner.getMaxHealth()) > 0.4 && this.taskOwner.tickCount % 40 != 0) {
         return false;
      } else if (this.taskOwner.getLastHurtByMob() == null && !this.taskOwner.isSleeping() && this.taskOwner.getCommandInt() == 0 && !this.taskOwner.isTame()) {
         ...
         if (list.isEmpty()) {
            ...
            Vec3 vec3d = DefaultRandomPos.getPosAway(
               this.taskOwner, 16, 7, new Vec3(this.toAvoid.getX(), this.toAvoid.getY(), this.toAvoid.getZ())
            );
            if (vec3d == null) {
               return false;
