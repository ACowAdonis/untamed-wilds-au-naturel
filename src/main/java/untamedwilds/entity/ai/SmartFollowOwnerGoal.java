...
public class SmartFollowOwnerGoal extends Goal {
   ...
   public SmartFollowOwnerGoal(ComplexMob entityIn, double speedIn, float minDistIn, float maxDistIn) {
      ...
      this.level = this.taskOwner.level();
   ...
   public void tick() {
      ...
      if (this.taskOwner.canMove() && this.taskOwner.getCommandInt() == 1 && --this.timeToRecalcPath <= 0) {
         ...
         if (!this.taskOwner.isLeashed()
            && !this.taskOwner.isPassenger()
            && !this.navigator.moveTo(this.owner, this.followSpeed)
            && this.taskOwner.distanceToSqr(this.owner) >= 225.0
   ...
   private boolean maybeTeleportTo(int p_25304_, int p_25305_, int p_25306_) {
      if (Math.abs((double)p_25304_ - this.owner.getX()) < 2.0 && Math.abs((double)p_25306_ - this.owner.getZ()) < 2.0) {
         ...
         this.taskOwner.moveTo((double)p_25304_ + 0.5, (double)p_25305_, (double)p_25306_ + 0.5, this.taskOwner.getYRot(), this.taskOwner.getXRot());
   ...
   private boolean canTeleportTo(BlockPos p_25308_) {
      ...
      if (blockpathtypes != BlockPathTypes.WALKABLE) {
         ...
         if (blockstate.getBlock() instanceof LeavesBlock) {
            ...
            return this.level.noCollision(this.taskOwner, this.taskOwner.getBoundingBox().inflate(1.0, 0.0, 1.0).move(blockpos));
         }
      }
   }
