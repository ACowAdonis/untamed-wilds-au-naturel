...
public class SmartSwimmingMoveControl extends MoveControl {
   ...
   public SmartSwimmingMoveControl(Mob entityIn, int maxTurnX, int maxTurnY, float waterSpeedFactor, float landSpeedFactor, boolean applyGravity) {
      super(entityIn);
         ...
         if (d3 < 2.5000003E-7F) {
            ...
            if (this.mob.isInWater()) {
               ...
               if (this.mob.getAirSupply() < 40) {
                  this.mob.setDeltaMovement(this.mob.getDeltaMovement().add(0.0, 0.15, 0.0));
               }

               float f4 = Mth.cos(this.mob.getXRot() * (float) (Math.PI / 180.0));
               float f3 = Mth.sin(this.mob.getXRot() * (float) (Math.PI / 180.0));
               this.mob.zza = f4 * f1;
               this.mob.yya = -f3 * f1;
            } else {
