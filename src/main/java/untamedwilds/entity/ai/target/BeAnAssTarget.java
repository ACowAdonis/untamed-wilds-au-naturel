...
public class BeAnAssTarget<T extends LivingEntity> extends TargetGoal {
   ...
   protected final BeAnAssTarget.Sorter sorter;
   ...
   public BeAnAssTarget(ComplexMob creature, Class<T> classTarget, boolean checkSight) {
      ...
      this.sorter = new BeAnAssTarget.Sorter(creature);
      this.setFlags(EnumSet.of(Flag.TARGET));
      this.targetEntitySelector = entity -> {
         if (!(entity instanceof Creeper) && ComplexMob.getEcoLevel(entity) <= ComplexMob.getEcoLevel(this.mob)) {
            if (this.mob.getClass() == entity.getClass() && this.mob instanceof ISpecies && entity instanceof ISpecies) {
               ComplexMob attacker = (ComplexMob)this.mob;
   ...
   public boolean canUse() {
      if (!(this.mob instanceof RangedAttackMob)) {
         ...
         return false;
      } else if (this.mob.getRandom().nextInt(200) == 0 && !this.mob.isBaby() && !(this.mob.getHealth() < this.mob.getMaxHealth() / 2.0F)) {
         if (this.mob instanceof ComplexMob complexMob && complexMob.isTame()) {
            return false;
         }
