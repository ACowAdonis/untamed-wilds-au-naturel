...
public class BisonTerritorialityFight extends Goal {
   ...
   public boolean canUse() {
      if (EntityUtils.hasFullHealth(this.taskOwner) && !this.taskOwner.isBaby() && this.taskOwner.canMove() && this.taskOwner.isMale()) {
         if (this.taskOwner.getRandom().nextInt(400) != 0) {
            ...
            List<? extends EntityBison> list = this.taskOwner.level().getEntitiesOfClass(this.taskOwner.getClass(), this.taskOwner.getBoundingBox().inflate(8.0, 4.0, 8.0));
            EntityBison target_animal = null;
            double d0 = Double.MAX_VALUE;

            for (EntityBison potential_targets : list) {
               if (!potential_targets.equals(this.taskOwner)
                  && !potential_targets.isBaby()
                  && potential_targets.getVariant() == this.taskOwner.getVariant()
                  && potential_targets.isMale()
                  && potential_targets.canMove()) {
            ...
            if (target_animal == null) {
               ...
               this.isDone = false;
               this.slaveGoal = new BisonTerritorialityFight.TerritorialFightingGoal_slave(this.targetAnimal, 1.4F, this.taskOwner);
               return true;
            }
         }
      } else {
   ...
   public void tick() {
      ...
      if (this.charge > 0) {
         ...
         for (LivingEntity entityHit : this.taskOwner.level().getNearbyEntities(LivingEntity.class, TargetingConditions.forCombat(), this.taskOwner, offset_box)) {
            if (!entityHit.equals(this.taskOwner) && this.taskOwner.hasLineOfSight(entityHit) && entityHit.equals(this.targetAnimal)) {
               RandomSource rand = this.taskOwner.getRandom();
               ServerLevel world = (ServerLevel)this.taskOwner.level();
               world.sendParticles(
                  ParticleTypes.SMOKE,
               ...
               this.targetAnimal.hurt(this.taskOwner.damageSources().noAggroMobAttack(this.taskOwner), 2.0F);
               this.isDone = true;
            }
         }
      }
   ...
   public void stop() {
      ...
      this.taskOwner
         .level()
         .playSound(null, this.taskOwner.getX(), this.taskOwner.getY(), this.taskOwner.getZ(), SoundEvents.GOAT_RAM_IMPACT, SoundSource.NEUTRAL, 1.0F, 1.0F);
