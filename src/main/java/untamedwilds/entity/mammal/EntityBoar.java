...
import untamedwilds.entity.ai.RaidCropsGoal;
...
public class EntityBoar extends ComplexMobTerrestrial implements ISpecies, INewSkins, INeedsPostUpdate {
   ...
   public void registerGoals() {
      ...
      this.goalSelector.addGoal(2, new EntityBoar.GoHogWildGoal(this, 2.0));
      ...
      this.goalSelector.addGoal(5, new SmartWanderGoal(this, 1.0, 120, 20, true));
   ...
   public void aiStep() {
      if (!this.level().isClientSide) {
         ...
         if (this.getAnimation() != NO_ANIMATION && this.getAnimation() == ATTACK && this.getAnimationTick() == 8 && this.random.nextInt(3) == 0) {
   ...
   public EntityBoar getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
      return this.create_offspring(new EntityBoar((EntityType<? extends ComplexMob>)ModEntity.BOAR.get(), this.level()));
   }
   ...
   public boolean isWarthog() {
      return (Boolean)this.entityData.get(WARTHOG);
   }

   private void setWarthog(boolean warthog) {
      this.entityData.set(WARTHOG, warthog);
   }
   ...
   public static class GoHogWildGoal extends Goal {
      ...
      public void tick() {
         if (this.boar.getRandom().nextInt(this.adjustedTickDelay(50)) < 3) {
            this.boar.setAnimation(EntityBoar.ATTACK);
         ...
         if (!this.boar.isTame() && this.boar.getRandom().nextInt(this.adjustedTickDelay(50)) == 0) {
            Entity entity = (Entity)this.boar.getPassengers().get(0);
            if (entity == null) {
               return;
            }
