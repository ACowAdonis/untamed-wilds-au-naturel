...
public class EntityManatee extends ComplexMobAquatic implements ISpecies, INewSkins, INeedsPostUpdate {
   ...
   protected void registerGoals() {
      ...
      this.goalSelector.addGoal(3, new EntityManatee.ManateeFeedGoal(this, 120));
   ...
   public void aiStep() {
      ...
      if (!this.level().isClientSide) {
         if (this.isEating()) {
            this.setDeltaMovement(this.getDeltaMovement().add(0.0, -0.01F, 0.0));
         }
   ...
   public boolean wantsToBreed() {
      if ((Boolean)ConfigGamerules.naturalBreeding.get() && this.getAge() == 0 && EntityUtils.hasFullHealth(this)) {
         List<EntityManatee> list = this.level().getEntitiesOfClass(EntityManatee.class, this.getBoundingBox().inflate(12.0, 8.0, 12.0));
   ...
   public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageableEntity) {
      return this.create_offspring(new EntityManatee((EntityType<? extends ComplexMob>)ModEntity.MANATEE.get(), this.level()));
   }
   ...
   public class ManateeFeedGoal extends Goal {
      ...
      public boolean canUse() {
         if (this.taskOwner.getRandom().nextInt(this.chance) == 0) {
            ...
            this.targetPos = this.getNearbySandBlock(this.taskOwner.blockPosition());
      ...
      private BlockPos getNearbySandBlock(BlockPos roomCenter) {
         ...
         for (BlockPos blockpos : MutableBlockPos.betweenClosed(roomCenter.offset(-X, -Y, -X), roomCenter.offset(X, 0, X))) {
            if (this.taskOwner.level().getBlockState(blockpos).is(ModTags.ModBlockTags.GRAZEABLE_ALGAE) && EntityManatee.this.random.nextInt(2) == 0) {
               return blockpos;
            }
         }
      ...
      public void tick() {
         ...
         if (this.counter == 0
            && this.taskOwner
                  .distanceToSqr((double)this.targetPos.getX() + 0.5, (double)this.targetPos.above().getY(), (double)this.targetPos.getZ() + 0.5)
               < 1.0) {
         ...
         if (this.counter > 0) {
            ...
            if (this.counter == 0) {
               ...
               this.taskOwner.getLookControl().setLookAt((double)this.targetPos.getX(), (double)this.targetPos.getY(), (double)this.targetPos.getZ());
               if (worldIn.getBlockState(this.targetPos).is(ModTags.ModBlockTags.GRAZEABLE_ALGAE)
                  && ForgeEventFactory.getMobGriefingEvent(worldIn, this.taskOwner)
                  && (Boolean)ConfigGamerules.grazerGriefing.get()) {
                  worldIn.destroyBlock(this.targetPos, false);
               }

               this.taskOwner.ate();
               this.taskOwner.playSound(SoundEvents.GENERIC_EAT, 1.0F, 1.0F);
