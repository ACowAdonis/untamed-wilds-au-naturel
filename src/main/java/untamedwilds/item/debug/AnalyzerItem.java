...
public class AnalyzerItem extends Item {
   ...
   public InteractionResult interactLivingEntity(ItemStack stack, Player playerIn, LivingEntity target, InteractionHand hand) {
      ...
      if (world.isClientSide) {
      ...
      } else if (target instanceof ComplexMob entity) {
         ...
         playerIn.sendSystemMessage(
            MutableComponent.create(
               new LiteralContents(
                  "Diagnose: "
                     + (ConfigGamerules.genderedBreeding.get() ? entity.getGenderString() + " " : "")
                     + entityName
                     + " (Skin: "
                     + entity.getSkin()
                     + ") (Eco Level: "
                     + ComplexMob.getEcoLevel(entity)
                     + ") "
                     + entity.getHealth()
                     + "/"
                     + entity.getMaxHealth()
         ...
         if ((Boolean)ConfigGamerules.scientificNames.get()) {
            ...
            playerIn.sendSystemMessage(Component.translatable(entity.getType().getDescriptionId() + useVarName + ".sciname").withStyle(ChatFormatting.ITALIC));
         }
         ...
         if (!entity.isMale() && entity.getAge() > 0 && !(Boolean)ConfigGamerules.easyBreeding.get()) {
            playerIn.sendSystemMessage(
               MutableComponent.create(
                  new LiteralContents(
                     "This female will give birth in " + TimeUtils.convertTicksToDays(world, entity.getAge()) + " (" + entity.getAge() + " ticks)"
                  )
               )
            );
         }
         ...
         if (entity.isBaby()) {
            playerIn.sendSystemMessage(
               MutableComponent.create(
                  new LiteralContents(
                     "This mob will grow up in " + TimeUtils.convertTicksToDays(world, entity.getAge() * -1) + " (" + entity.getAge() * -1 + " ticks)"
                  )
               )
            );
         }
            ...
            MutableComponent.create(
               new LiteralContents(
                  "Diagnose: "
                     ...
                     + target.getMaxHealth()
                     + " HP (Eco Level: "
                     + ComplexMob.getEcoLevel(target)
                     + ")"
               )
            )
         );
         if (target.isBaby() && target instanceof AgeableMob entity) {
            playerIn.sendSystemMessage(
               MutableComponent.create(
                  new LiteralContents(
                     "This mob will grow up in " + TimeUtils.convertTicksToDays(world, entity.getAge() * -1) + " (" + entity.getAge() * -1 + " ticks)"
