...
public abstract class FaunaHandler {
   private static final List<FaunaHandler.SpawnListEntry> spawnCritter = Lists.newArrayList();
   private static final List<FaunaHandler.SpawnListEntry> spawnUndergroundLarge = Lists.newArrayList();
   private static final List<FaunaHandler.SpawnListEntry> spawnLargePrey = Lists.newArrayList();
   private static final List<FaunaHandler.SpawnListEntry> spawnDenseWater = Lists.newArrayList();
   private static final List<FaunaHandler.SpawnListEntry> spawnOcean = Lists.newArrayList();
   private static final List<FaunaHandler.SpawnListEntry> spawnSessile = Lists.newArrayList();
   private static final List<FaunaHandler.SpawnListEntry> spawnApexPred = Lists.newArrayList();
   private static final List<FaunaHandler.SpawnListEntry> spawnFeeder = Lists.newArrayList();

   public static List<FaunaHandler.SpawnListEntry> getSpawnableList(FaunaHandler.animalType type) {
      return switch (type) {
         ...
         case FEEDER -> spawnFeeder;
      };
   }

   public static List<FaunaHandler.SpawnListEntry> getSpawnableList(String type) {
      switch (type) {
         case "critter":
            return spawnCritter;
         case "benthos":
   ...
   public static class SpawnListEntry extends IntrusiveBase {
      public static final Codec<FaunaHandler.SpawnListEntry> CODEC = RecordCodecBuilder.create(
      ...
      public SpawnListEntry(String entityName, Integer weight, Integer minGroupCount, Integer maxGroupCount) {
         super(weight);
         this.entityType = (EntityType<?>)ForgeRegistries.ENTITY_TYPES.getValue(new ResourceLocation(entityName));
      ...
      public Integer getGroupCount() {
         if (this.maxGroupCount <= 0 || this.minGroupCount >= this.maxGroupCount) {
            return this.minGroupCount;
         }
         return this.minGroupCount + new Random().nextInt(this.maxGroupCount - this.minGroupCount + 1);
      }
   }
