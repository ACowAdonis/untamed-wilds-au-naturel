...
public class FeatureApexPredators extends Feature<NoneFeatureConfiguration> {
   public FeatureApexPredators(Codec<NoneFeatureConfiguration> codec) {
      super(codec);
   }
      ...
      if (((List)ConfigMobControl.dimensionBlacklist.get()).contains(world.getLevel().dimension().location().toString())) {
         ...
         for (int i = 0; i < 3; i++) {
            ...
            if (entry.isPresent()) {
               ...
               if (type != null
                  && FaunaSpawn.performWorldGenSpawning(type, Type.NO_RESTRICTIONS, Types.WORLD_SURFACE_WG, world, pos, rand, entry.get().getGroupCount())) {
                  return true;
               }
            }
         }
